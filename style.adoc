...
summary: Nuestro estilo es conversacional y empático, pero nos mantenemos profesionales y llegar al punto. 
---
...
.Admoniciones estándar
Tres admoniciones estándar utilizan etiquetas personalizadas. Las etiquetas son NOTA, TIP y PRECAUCIÓN. Estas tres admoniciones estándar tienen un formato distinto al texto normal, y sus etiquetas siempre están escritas en mayúsculas en la fuente de AsciiDoc.
...
* Consejo Utilice EL CONSEJO para proporcionar consejos útiles o accesos directos que pueden mejorar la experiencia del usuario. Por ejemplo, un CONSEJO puede ayudar a un usuario a completar un paso o tarea de forma más fácil y eficiente. Un CONSEJO debe usarse con moderación, si es que lo hacemos, ya que nuestra política es documentar la mejor manera de completar una tarea por defecto.
...
.Admonición a las mejores prácticas
La admonición Best Practice no es una etiqueta de admonición personalizada, sino que se puede utilizar como una convención de formato independiente. Utilice la práctica recomendada para resaltar formas óptimas de completar tareas o utilizar un producto. No son meras sugerencias, sino estrategias que han sido validadas por expertos o estándares de la industria.

* *¿Qué hace una mejor práctica?*
+
Se trata de una estrategia práctica y específica para tareas que ofrece ventajas claras y está respaldada por fuentes fiables.

* * ¿Cuándo puedo usar las mejores prácticas?*
+
Puede utilizar las mejores prácticas para todos los tipos de contenido y todos los públicos. Como CONSEJOS, úsalos con moderación para mantener su significado.

* * ¿Cómo formateo las mejores prácticas?*
+
Para utilizar el formato de mejores prácticas, aplique mayúsculas de estilo de oración, negrita el término Best Practice, seguido de dos puntos y un espacio.

+
Presente las mejores prácticas en un formato coherente y fácil de usar. Esto podría ser una lista con viñetas, una lista numerada o un párrafo, dependiendo del contexto. Por ejemplo, *Best Practice*: Pruebe siempre los cambios de configuración en un entorno de estadificación antes de aplicarlos a la producción.
...
Una interfaz de programación de aplicaciones (API) se refiere a una interfaz única que proporciona acceso a un producto o servicio específico. Dentro de una API de producto grande, utilice el término _API_ para hacer referencia a cada conjunto de puntos finales asociados a un tipo de recurso o componente. Cuando haga referencia a varias interfaces distintas, utilice el término _APIs_.
